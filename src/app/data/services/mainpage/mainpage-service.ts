...
export class MainpageService {
  ...
  getNowPlaying(): Observable<IApiResponse> {
    return this.http.get<IApiResponse>(
      `${this.apiUrl}/movie/now_playing${this.languages}`,
      {
  ...
  getTrendingMovie(time: string) {
    return this.http.get(
      `${this.apiUrl}/trending/movie/${time}${this.languages}`,
      { headers: this.headers }
    );
  }
  getTrendingTV(time: string) {
    return this.http.get(
      `${this.apiUrl}/trending/tv/${time}${this.languages}`,
      { headers: this.headers }
    );
  }
  getTrendingPeople(time: string) {
    return this.http.get(
      `${this.apiUrl}/trending/person/${time}${this.languages}`,
      { headers: this.headers }
    );
  }
  getPopularList(): Observable<IApiResponse> {
    return this.http.get<IApiResponse>(
      `${this.apiUrl}/movie/popular${this.languages}`,
      {
        headers: this.headers,
      }
    );
  ...
  getTVGenres() {
    return this.http.get(
      `${this.apiUrl}/discover/tv?with_genres=16&without_genres=10762&without_genres=10751&with_original_language=ja`,
      {
        headers: this.headers,
      }
    );
  ...
  getListGenres() {
    return this.http.get(`${this.apiUrl}/genre/movie/list${this.languages}`, {
      headers: this.headers,
    });
  }
}
